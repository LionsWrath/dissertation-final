...
% \newfloat[chapter]{algoritmo}{loa}{Algorithm}
\newcommand{\listaalgoritmosname}{List of Algorithms} %% Título da lista
\newlistof{listadealgoritmos}{loa}{\listaalgoritmosname}
\newlistentry{algoritmo}{loa}{0} %% Nome do ambiente de cada algoritmo, e.g.
...
%where the maximum delay could be measured by calculating the diameter of the network
...
with swaps restricted to categories (a) and (b) will have less swaps than a 
swap sequence that solves the same instance and uses swaps of category (c) 
\textemdash as \cite{Smith:1999} tried to prove, but subsequently found an 
error~\cite{Smith:2011:Corrigendum}.
Then, \cite{Vaughan:1991} conjectured that any optimal swap sequence would 
not swap already correct tokens on leafs and called them \textit{happy leafs}, 
resulting in the so-called \textit{Happy Leaf Conjecture}
%\footnote{The original  paper actually claimed a stronger conjecture with the Happy Leaf Conjecture as a special case.}.
This conjecture was disproved by \cite{Ahmad:2019}, as they shown that there is 
a class of infinite trees that need (c) swaps to achieve an optimal solution and 
that any algorithm that do not consider swaps on happy leafs has an approximation 
factor of \textit{at least} $\frac{4}{3}$ for general graphs and trees.
...
% NO PAPER PARAGRAPHS IN THE REST!
...
% \footnote{Seb: Mudei alguma coisa. Introduzi T para ficar mais claro.}
% Ok! Anteriormente não tinha utilizado T porque era uma das variáveis dos modelos.
...
Now we consider a one-cycle $\cycle$ of size at least 3. Let $\subtree$ be the subtree of $\cotreeOf{\graphG}$ rooted at $\lca{\cycle}$. %containing only children that have at least one vertex of $\cycle$.
...
% \footnote{Seb:estava subtree mas acho que estava errado, por favor verificar em todo o texto quando aparece subtree}
...
% Now, some exploration can be done about the strength of this characterization
% and how small are these values of swaps for these cycles. \footnote{seb: Melhorar.}
...
% \footnote{Notação LCA: $\lca{\cycle}$ ou $\lcaOf{\applyFunc{\conflictGraph}{\graphG}}{\vertexFunc{\cycle}}$}
...
\sizeof{\applyFunc{\zeroCycles}{\conflictGraph_{\mapFunc}}}$.
\end{lemma}
% \footnote{Seb: A gente definiu este $OPT_G$? Caio: Pelo jeito não, vou definir.}
\begin{proof}
Directly from Lemma~\ref{lem:m_onecycle} and Lemma~\ref{lem:m_zerocycle}.
\end{proof}
...
\begin{proof}
    ...
    % substituting $\applyFunc{p}{\graphG, 
    % \mapFunc_{k}}$ for a $0$, as it is the last configuration and this swap sequence solve the
    % instance, and substituting $\applyFunc{p}{\graphG, \mapFunc_{0}}$ by the original assumption,
    % the Inequation~\ref{equ:lower_bound} is found.
...
\begin{enumerate}
    \item Adding a vertex connected by edges to the graph $\graphG$ can increase the size of the matching $\sizeof{\matchingOf{\graphG}}$ in at most one; %\label{lem:matching_inc_one:1}
...
% \footnote{Seb: Se acharem necessário passem o algoritmo a padrão algoritmo usando algum pacote.}
\end{proof}
